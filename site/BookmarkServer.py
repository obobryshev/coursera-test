...
def CheckURI(uri, timeout=5):
    ...
    (i.e. times out).
    '''
    try:     # 1. Write this function.  Delete the following line.
...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            if name in memory:  # 2. Send a 303 redirect to the long URI in memory[name].
                self.send_response(303)  # We know that name! Send a redirect to it.
    ...
    def do_POST(self):
        ...
        if CheckURI(longuri):
            memory[shortname] = longuri # This URI is good!  Remember it under the specified name.
            # 4. Serve a redirect to the root page (the form). Delete the following line.
